...
public class SaleService(
    ...
    IMedicineService _medicineService) : ISaleService
{
    ...
    {
        if (await _context.Sales.FirstOrDefaultAsync(s => s.Id == id) == null)
        {
            return false;
        }
        return true;
    ...
    {
        return await _context.Sales.FirstOrDefaultAsync(s => s.Id == id) ?? throw new ArgumentException("Unexisting sale");
    }
    ...
    public async Task CheckSaleDates(IEnumerable<MedicineServiceModel> medicines)
    {
        foreach (var medicineModel in medicines)
        {
            if (medicineModel.SaleId.HasValue)
            {
                ...
                var medicine = await _medicineService.FindByIdAsync(medicineModel.Id);//to apply the changes on the database
    ...
    public async Task AttachMedicine(int saleId, int medicineId)
    {
        ...
        var medicine = await _medicineService.FindByIdAsync(medicineId);
